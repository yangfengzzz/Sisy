...
#include "../../src.gfx/Vao/OgreAsyncTicket.h"
#include "../../src.gfx/Vao/OgreTexBufferPacked.h"
#include "../../src.gfx/Vao/OgreUavBufferPacked.h"
...
namespace Demo
{
														...
														size_t srcNumElements )
	{
		...
		for( size_t i=0u; i<srcNumElements; ++i )
		{
			...
			if( referenceBuffer[refIdx] != dataBuffer[uavIdx] )
			{
				...
				txt.a( "Failed referenceBuffer[i] != dataBuffer[i]. Expecting ",
					   referenceBuffer[refIdx], " got ", dataBuffer[uavIdx] );
	...
	void UnalignedBufferUavCopyGameState::createScene01(void)
    {
		...
		{
			indexBuffer[i] = vaoManager->createIndexBuffer( Ogre::IndexBufferPacked::IT_16BIT,
															c_refBufferCount,
															Ogre::BT_DEFAULT,
															referenceBuffer[i], false );
		}
		Ogre::UavBufferPacked *uavBuffer =
		...
		//actually working.
		verifyBuffer( referenceBuffer[idx], indexBuffer[idx], uavBuffer, 0u, 0u, c_refBufferCount ); idx = !idx;
		verifyBuffer( referenceBuffer[idx], indexBuffer[idx], uavBuffer, 0u, 0u, c_refBufferCount ); idx = !idx;

		verifyBuffer( referenceBuffer[idx], indexBuffer[idx], uavBuffer, 0u, 1u, 7u ); idx = !idx;

		verifyBuffer( referenceBuffer[idx], indexBuffer[idx], uavBuffer, 5u, 1u, 7u ); idx = !idx;
		verifyBuffer( referenceBuffer[idx], indexBuffer[idx], uavBuffer, 5u, 1u, 7u ); idx = !idx;
